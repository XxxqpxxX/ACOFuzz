...
class PolicyRandom(PolicyBase):
    ...
    def select_tx(self, obs, methodname, arguments):
    # def select_tx(self, obs, methodname):
    # def select_tx(self, obs):
        contract = self._select_contract()
        address = contract.addresses[0]
        method = self._select_method(contract)
        sender = self._select_sender()
        argument = self._select_arguments(contract, method, sender, obs)
        amount = self._select_amount(contract, method, sender, obs)
        timestamp = self._select_timestamp(obs)
        if arguments == ['']:
            arguments = []
        ...
        tx = Tx(self, contract.name, address, methodname, bytes(), arguments, amount, sender, timestamp, True)
        # tx = Tx(self, contract.name, address, methodname[0], bytes(), arguments, amount, sender, timestamp, True)
        # return contract, address, amount, sender, timestamp
        return tx
